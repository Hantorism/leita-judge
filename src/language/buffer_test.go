...
func TestMemoryLimitWithBuffer(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "RUST는 +16MB", language: "RUST", limitKB: limitKB, want: limitKB + 16*1024},
		{name: "CS는 +64MB", language: "CS", limitKB: limitKB, want: limitKB + 64*1024},
		{name: "TYPESCRIPT는 +32MB (JS와 동일)", language: "TYPESCRIPT", limitKB: limitKB, want: limitKB + 32*1024},
		{name: "미지원 언어는 보정 없음", language: "HASKELL", limitKB: limitKB, want: limitKB},
...
func TestTimeLimitWithBuffer(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "RUST는 보정 없음 (네이티브, 실측 1.0×)", language: "RUST", limitMs: limitMs, want: 1000},
		{name: "CS는 ×2+1초", language: "CS", limitMs: limitMs, want: 3000},
		{name: "TYPESCRIPT는 ×3+2초 (JS와 동일)", language: "TYPESCRIPT", limitMs: limitMs, want: 5000},
		{name: "미지원 언어는 보정 없음 (배수 0이 되면 안 됨)", language: "HASKELL", limitMs: limitMs, want: 1000},
